...
int main(int argc, char* argv[]) {
    ...
    const bool should_start_gui = (has_command_line_argument && show_gui) || !has_command_line_argument;
    const bool should_show_boot_screen =
        Config::showBootScreen() && should_start_gui && !has_game_argument;

    if (should_show_boot_screen) {
        const QSize boot_size(static_cast<int>(Config::getWindowWidth()),
                              static_cast<int>(Config::getWindowHeight()));
        std::unique_ptr<BootScreen> boot_screen = std::make_unique<BootScreen>(boot_size);
        QEventLoop boot_loop;
        QObject::connect(boot_screen.get(), &BootScreen::bootFinished, &boot_loop, &QEventLoop::quit);
        boot_screen->show();
        boot_loop.exec();
        boot_screen.reset();
    }
