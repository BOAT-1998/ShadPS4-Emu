...
GameListFrame::GameListFrame(std::shared_ptr<gui_settings> gui_settings,
    ...
    connect(this, &QTableWidget::cellDoubleClicked, this, [this](int row, int column) {
        Q_UNUSED(column);
        if (m_game_info && row >= 0 && row < m_game_info->m_games.size()) {
            emit GameSelected(m_game_info->m_games[row]);
        }
    });
}
...
void GameListFrame::onCurrentCellChanged(int currentRow, int currentColumn, int previousRow,
    ...
    if (m_game_info && currentRow >= 0 && currentRow < m_game_info->m_games.size()) {
        emit GameSelected(m_game_info->m_games[currentRow]);
    }
}

void GameListFrame::PlayBackgroundMusic(QTableWidgetItem* item) {
    if (!Config::getLauncherBgmEnabled()) {
