...
SettingsDialog::SettingsDialog(std::shared_ptr<gui_settings> gui_settings,
    ...
    connect(ui->applyRtxPresetButton, &QPushButton::clicked, this, &SettingsDialog::ApplyRtxPreset);
    ...
    presentModeMap = {{tr("Mailbox (Vsync)"), "Mailbox"},
                      {tr("Fifo (Vsync)"), "Fifo"},
                      {tr("Fifo Relaxed (Adaptive Vsync)"), "FifoRelaxed"},
    ...
    // GUI TAB
    {
        ...
        connect(ui->uiSfxCheckBox, &QCheckBox::toggled, this, [this](bool enabled) {
            Config::setUiSoundEffectsEnabled(enabled);
            emit UiSoundEffectsToggled(enabled);
        });

        connect(ui->playBGMCheckBox, &QCheckBox::toggled, this, [this](bool enabled) {
            Config::setLauncherBgmEnabled(enabled);
            ui->BGMVolumeSlider->setEnabled(enabled);
            if (!enabled) {
                BackgroundMusicPlayer::getInstance().stopMusic();
            }
            emit LauncherBgmToggled(enabled);
        });

        connect(ui->autoScrollLogCheckBox, &QCheckBox::toggled, this, [this](bool enabled) {
            m_gui_settings->SetValue(gui::mw_log_auto_scroll, enabled);
            emit AutoScrollLogToggled(enabled);
        });
    ...
    connect(ui->sharpenCheckBox, &QCheckBox::toggled, ui->RCASCheckBox, &QCheckBox::setChecked);
    connect(ui->RCASCheckBox, &QCheckBox::toggled, ui->sharpenCheckBox, &QCheckBox::setChecked);
    connect(ui->autoDetectButton, &QPushButton::clicked, this, [this]() {
        Config::AutoTuneFromHardware(is_game_specific);
        LoadValuesFromConfig();
        UpdateSettings(is_game_specific);
    ...
    // Descriptions
    {
        ...
        ui->showBootScreenCheckBox->installEventFilter(this);
        ...
        ui->renderQualityGroupBox->installEventFilter(this);
        ui->graphicsProfileComboBox->installEventFilter(this);
        ui->resolutionScaleSpinBox->installEventFilter(this);
        ui->textureQualityComboBox->installEventFilter(this);
        ui->anisotropicComboBox->installEventFilter(this);
        ui->frameLimitComboBox->installEventFilter(this);
        ui->fxaaCheckBox->installEventFilter(this);
        ui->sharpenCheckBox->installEventFilter(this);
        ui->autoDetectButton->installEventFilter(this);
...
void SettingsDialog::LoadValuesFromConfig() {
    ...
    if (!is_game_specific) {
        ...
        ui->uiSfxCheckBox->setChecked(Config::getUiSoundEffectsEnabled());
        ui->playBGMCheckBox->setChecked(Config::getLauncherBgmEnabled());
        ...
        ui->BGMVolumeSlider->setEnabled(ui->playBGMCheckBox->isChecked());
    ...
    ui->showBootScreenCheckBox->setChecked(
        toml::find_or<bool>(data, "General", "showBootScreen", true));
    ...
    QString translatedText_PresentMode = presentModeMap.key(
        QString::fromStdString(presentMode), tr("Mailbox (Vsync)"));
    ...
    ui->autoScrollLogCheckBox->setChecked(
...
void SettingsDialog::updateNoteTextEdit(const QString& elementName) {
    ...
    } else if (elementName == "showBootScreenCheckBox") {
        text = tr("Show PS4 Boot Screen:\\nDisplays a PS4-style boot screen before the main window appears. Disable to open the launcher immediately.");
    ...
    } else if (elementName == "renderQualityGroupBox" || elementName == "resolutionScaleSpinBox" ||
               elementName == "graphicsProfileComboBox") {
        text = tr("Rendering Quality:\\nControls internal resolution and PS4/Pro style presets. Higher scales improve clarity but increase GPU load. Profiles tune multiple settings together.");
    } else if (elementName == "textureQualityComboBox") {
        text = tr("Texture Quality:\\nControls the texture mip bias. Lower settings reduce VRAM usage but look blurrier.");
    } else if (elementName == "anisotropicComboBox") {
        text = tr("Anisotropic Filtering:\\nImproves texture detail when viewed at oblique angles. Higher values cost more GPU bandwidth.");
    } else if (elementName == "frameLimitComboBox") {
        text = tr("Frame Limit:\\nCaps the maximum FPS to smooth frame pacing and reduce GPU power when running faster than the target.");
    } else if (elementName == "fxaaCheckBox") {
        text = tr("FXAA:\\nFast post-process anti-aliasing to reduce jagged edges with minimal performance impact.");
    } else if (elementName == "sharpenCheckBox") {
        text = tr("Sharpen:\\nEnables RCAS-style sharpening after upscaling to restore detail.");
    } else if (elementName == "autoDetectButton") {
        text = tr("Auto Detect:\\nReads your GPU/CPU and applies a tuned preset targeting PS4 Pro visuals when possible.");
    }
...
void SettingsDialog::UpdateSettings(bool is_specific) {
    ...
    m_gui_settings->SetValue(gui::mw_log_auto_scroll, ui->autoScrollLogCheckBox->isChecked());
    ...
    Config::setGraphicsProfile(ui->graphicsProfileComboBox->currentText().toStdString(),
                               is_specific);
    Config::setResolutionScale(ui->resolutionScaleSpinBox->value(), is_specific);
    Config::setTextureQuality(ui->textureQualityComboBox->currentText().toStdString(),
                              is_specific);
    int aniso_level = 1;
    const auto aniso_text = ui->anisotropicComboBox->currentText();
    if (aniso_text.contains("2x")) {
        aniso_level = 2;
    } else if (aniso_text.contains("4x")) {
        aniso_level = 4;
    } else if (aniso_text.contains("8x")) {
        aniso_level = 8;
    } else if (aniso_text.contains("16x")) {
        aniso_level = 16;
    }
    Config::setAnisotropicLevel(aniso_level, is_specific);
    int frame_limit = 0;
    const auto frame_text = ui->frameLimitComboBox->currentText();
    if (frame_text.startsWith("30")) {
        frame_limit = 30;
    } else if (frame_text.startsWith("45")) {
        frame_limit = 45;
    } else if (frame_text.startsWith("60")) {
        frame_limit = 60;
    } else if (frame_text.startsWith("120")) {
        frame_limit = 120;
    } else if (frame_text.startsWith("144")) {
        frame_limit = 144;
    } else if (frame_text.startsWith("240")) {
        frame_limit = 240;
    }
    Config::setFrameLimit(frame_limit, is_specific);
    Config::setAsyncShaderCompile(ui->asyncShaderCompileCheckBox->isChecked(), is_specific);
    Config::setEnableMtRendering(ui->multiThreadRenderCheckBox->isChecked(), is_specific);
    Config::setShaderPreloadCache(ui->shaderPreloadCacheCheckBox->isChecked(), is_specific);
    Config::setEnableFxaa(ui->fxaaCheckBox->isChecked(), is_specific);
    Config::setEnableSharpen(ui->sharpenCheckBox->isChecked(), is_specific);
    ...
    Config::setRcasEnabled(ui->sharpenCheckBox->isChecked(), is_specific);
    ...
    Config::setShowBootScreen(ui->showBootScreenCheckBox->isChecked(), is_specific);
    ...
    if (!is_specific) {
        ...
        Config::setUiSoundEffectsEnabled(ui->uiSfxCheckBox->isChecked());
        Config::setLauncherBgmEnabled(ui->playBGMCheckBox->isChecked());
...
void SettingsDialog::ApplyRtxPreset() {
    // GPU-side tweaks
    ui->presentModeComboBox->setCurrentText(tr("Immediate (No Vsync)"));
    ui->resolutionScaleSpinBox->setValue(100);
    ui->textureQualityComboBox->setCurrentText(tr("High"));
    ui->anisotropicComboBox->setCurrentText(tr("8x"));
    ui->frameLimitComboBox->setCurrentText(tr("60 FPS"));
    ui->enableHDRCheckBox->setChecked(false);
    ui->graphicsProfileComboBox->setCurrentText(tr("PS4"));

    // Image quality
    ui->fxaaCheckBox->setChecked(true);
    ui->sharpenCheckBox->setChecked(false);
    ui->FSRCheckBox->setChecked(false);
    ui->RCASCheckBox->setChecked(false);
    ui->RCASSlider->setValue(250);

    // Performance / stutter reducers
    ui->asyncShaderCompileCheckBox->setChecked(true);
    ui->multiThreadRenderCheckBox->setChecked(true);
    ui->shaderPreloadCacheCheckBox->setChecked(true);
}
...
void SettingsDialog::SyncRealTimeWidgetstoConfig() {
    ...
    const int res_scale = toml::find_or<int>(
        gs_data, "Graphics", "resolutionScale", toml::find_or<int>(gs_data, "GPU", "resolutionScale", 100));
    ui->resolutionScaleSpinBox->setValue(res_scale);

    const std::string profile =
        toml::find_or<std::string>(gs_data, "Graphics", "profile", "PS4");
    ui->graphicsProfileComboBox->setCurrentText(QString::fromStdString(profile));

    const std::string texture =
        toml::find_or<std::string>(gs_data, "Graphics", "textureQuality", "High");
    ui->textureQualityComboBox->setCurrentText(QString::fromStdString(texture));

    const int aniso =
        toml::find_or<int>(gs_data, "Graphics", "anisotropicLevel", Config::getAnisotropicLevel());
    switch (aniso) {
    case 2:
        ui->anisotropicComboBox->setCurrentText("2x");
        break;
    case 4:
        ui->anisotropicComboBox->setCurrentText("4x");
        break;
    case 8:
        ui->anisotropicComboBox->setCurrentText("8x");
        break;
    case 16:
        ui->anisotropicComboBox->setCurrentText("16x");
        break;
    default:
        ui->anisotropicComboBox->setCurrentText("Off");
        break;
    }

    const int frame_limit =
        toml::find_or<int>(gs_data, "Graphics", "frameLimit", Config::getFrameLimit());
    if (frame_limit == 30) {
        ui->frameLimitComboBox->setCurrentText("30 FPS");
    } else if (frame_limit == 45) {
        ui->frameLimitComboBox->setCurrentText("45 FPS");
    } else if (frame_limit == 60) {
        ui->frameLimitComboBox->setCurrentText("60 FPS");
    } else if (frame_limit == 120) {
        ui->frameLimitComboBox->setCurrentText("120 FPS");
    } else if (frame_limit == 144) {
        ui->frameLimitComboBox->setCurrentText("144 FPS");
    } else if (frame_limit == 240) {
        ui->frameLimitComboBox->setCurrentText("240 FPS");
    } else {
        ui->frameLimitComboBox->setCurrentText("Unlimited");
    }

    const bool sharpen_enabled =
        toml::find_or<bool>(gs_data, "Graphics", "enableSharpen",
                            toml::find_or<bool>(gs_data, "GPU", "rcasEnabled", true));
    ui->sharpenCheckBox->setChecked(sharpen_enabled);
    ui->RCASCheckBox->setChecked(sharpen_enabled);

    const bool fxaa_enabled = toml::find_or<bool>(gs_data, "Graphics", "enableFxaa", false);
    ui->fxaaCheckBox->setChecked(fxaa_enabled);

    ui->asyncShaderCompileCheckBox->setChecked(
        toml::find_or<bool>(gs_data, "Graphics", "asyncShaderCompile", Config::getAsyncShaderCompile()));
    ui->multiThreadRenderCheckBox->setChecked(
        toml::find_or<bool>(gs_data, "Graphics", "enableMtRendering", Config::getEnableMtRendering()));
    ui->shaderPreloadCacheCheckBox->setChecked(
        toml::find_or<bool>(gs_data, "Graphics", "shaderPreloadCache", Config::getShaderPreloadCache()));

    if (presenter) {
        presenter->GetFsrSettingsRef().enable = ui->FSRCheckBox->isChecked();
        presenter->GetFsrSettingsRef().use_rcas = ui->sharpenCheckBox->isChecked();
