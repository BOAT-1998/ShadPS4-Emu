...
void Rasterizer::SetPredication(const AmdGpu::Liverpool::PredicationState& state) {
    predication = state;

    // If the device exposes conditional rendering we could map this to GPU side; for now we
    // evaluate on CPU and early-out draws via FilterDraw().
}
...
bool Rasterizer::FilterDraw() {
    ...
    if (predication.enabled &&
        (!instance.IsConditionalRenderingSupported() || predication.reference != 0)) {
        bool pass = true;
        const u8* ptr = reinterpret_cast<const u8*>(predication.address);
        if (ptr != nullptr) {
            u64 value = 0;
            if (predication.use_64bit) {
                std::memcpy(&value, ptr, sizeof(u64));
            } else {
                u32 val32 = 0;
                std::memcpy(&val32, ptr, sizeof(u32));
                value = val32;
            }
            pass = predication.draw_if_non_zero ? (value != predication.reference)
                                                : (value == predication.reference);
        }
        if (!pass) {
            ScopedMarkerInsert("PredicatedSkip");
            return false;
        }
    }
...
bool Rasterizer::BeginConditionalRendering() {
    if (!predication.enabled || predication.reference != 0 ||
        !instance.IsConditionalRenderingSupported()) {
        return false;
    }
    const u32 size = predication.use_64bit ? sizeof(u64) : sizeof(u32);
    const auto [buffer, base] = buffer_cache.ObtainBuffer(predication.address, size, false);
    const vk::ConditionalRenderingBeginInfoEXT info = {
        .buffer = buffer->Handle(),
        .offset = base,
        .flags = predication.draw_if_non_zero ? vk::ConditionalRenderingFlagsEXT{}
                                              : vk::ConditionalRenderingFlagBitsEXT::eInverted,
    };
    scheduler.CommandBuffer().beginConditionalRenderingEXT(info);
    return true;
}

void Rasterizer::EndConditionalRendering(const bool active) {
    if (active && instance.IsConditionalRenderingSupported()) {
...
void Rasterizer::Draw(bool is_indexed, u32 index_offset) {
    ...
    const bool conditional_active = BeginConditionalRendering();
    if (!FilterDraw()) {
        EndConditionalRendering(conditional_active);
        return;
    }
...
void Rasterizer::BindBuffers(const Shader::Info& stage, Shader::Backend::Bindings& binding,
    ...
    for (const auto& desc : stage.buffers) {
        ...
        if (!desc.IsSpecial() && vsharp.base_address != 0 && vsharp.GetSize() > 0) {
            ...
            const u64 base_adjust = is_storage ? 0u : (vsharp.base_address & 0xFu);
            const u64 base_addr = vsharp.base_address - base_adjust;
            info.offset_adjust = static_cast<u32>(base_adjust);
            info.size = memory->ClampRangeSize(vsharp.base_address, vsharp.GetSize());
            const auto lookup_size = info.size + base_adjust;
            info.id = buffer_cache.FindBuffer(base_addr, lookup_size);
        }
        buffer_bindings.emplace_back(info);
    }

    // Second pass to re-bind buffers that were updated after binding
    for (u32 i = 0; i < buffer_bindings.size(); i++) {
        const auto& binding_info = buffer_bindings[i];
        const auto& desc = stage.buffers[i];
        const bool is_storage = desc.IsStorage(binding_info.buffer);
        const u32 alignment =
        ...
        if (!binding_info.id) {
            ...
            const u64 base_addr = binding_info.buffer.base_address - binding_info.offset_adjust;
            const u64 lookup_size = binding_info.size + binding_info.offset_adjust;
            const auto [vk_buffer, offset] = buffer_cache.ObtainBuffer(
                base_addr, static_cast<u32>(lookup_size), desc.is_written, desc.is_formatted,
                binding_info.id);
            const u64 combined_offset = offset + binding_info.offset_adjust;
            const u32 offset_aligned = Common::AlignDown(combined_offset, alignment);
            const u32 adjust = static_cast<u32>(combined_offset - offset_aligned);
            ASSERT(adjust % 4 == 0);
            push_data.AddOffset(binding.buffer, adjust);
            buffer_infos.emplace_back(vk_buffer->Handle(), offset_aligned,
                                      binding_info.size + adjust);
            if (auto barrier =
            ...
            if (desc.is_written && desc.is_formatted) {
                texture_cache.InvalidateMemoryFromGPU(binding_info.buffer.base_address,
                                                      binding_info.size);
            }
        }
...
RenderState Rasterizer::BeginRendering(const GraphicsPipeline* pipeline) {
    ...
    if (auto image_id = db_desc.first; image_id) {
        ...
        const bool depth_writable = regs.depth_control.depth_enable &&
                                    regs.depth_control.depth_write_enable &&
                                    !regs.depth_render_control.depth_clear_enable;
        const bool stencil_writable =
            regs.depth_control.stencil_enable && regs.depth_buffer.StencilValid() &&
            !regs.depth_render_control.stencil_clear_enable;
        const bool needs_write = desc.view_info.is_storage || depth_writable || stencil_writable;
        const auto new_layout =
            needs_write ? (has_stencil ? vk::ImageLayout::eDepthStencilAttachmentOptimal
                                       : vk::ImageLayout::eDepthAttachmentOptimal)
                        : (has_stencil ? vk::ImageLayout::eDepthStencilReadOnlyOptimal
                                       : vk::ImageLayout::eDepthReadOnlyOptimal);
        const auto access =
            needs_write ? (vk::AccessFlagBits2::eDepthStencilAttachmentWrite |
