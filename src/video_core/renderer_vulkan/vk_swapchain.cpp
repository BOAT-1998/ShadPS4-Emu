...
void Swapchain::FindPresentMode() {
    ...
    auto requested_mode = Config::getPresentMode();
    std::ranges::transform(requested_mode.begin(), requested_mode.end(), requested_mode.begin(),
    ...
    } else if (requested_mode == "fiforelaxed") {
