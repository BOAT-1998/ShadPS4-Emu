...
Presenter::Presenter(Frontend::WindowSDL& window_, AmdGpu::Liverpool* liverpool_)
    ...
    fsr_settings.use_rcas = Config::getEnableSharpen();
...
void Presenter::Present(Frame* frame, bool is_reusing_frame) {
    ...
    frame_limiter.EndFrame(Config::getFrameLimit());
    free_frame();
    if (!is_reusing_frame) {
...
void Presenter::SetExpectedGameSize(s32 width, s32 height) {
    ...
    const int scale_percent = std::clamp(Config::getResolutionScale(), 50, 400);
    const float scale = static_cast<float>(scale_percent) * 0.01f;
    expected_frame_width = std::max(1, static_cast<s32>(expected_frame_width * scale));
    expected_frame_height = std::max(1, static_cast<s32>(expected_frame_height * scale));
}
