...
Sampler::Sampler(const Vulkan::Instance& instance, const AmdGpu::Sampler& sampler,
    ...
    const u32 config_aniso = static_cast<u32>(std::max(1, Config::getAnisotropicLevel()));
    const bool anisotropy_enable = instance.IsAnisotropicFilteringSupported() && config_aniso > 1;
    const float max_anisotropy =
        anisotropy_enable
            ? std::clamp(static_cast<float>(config_aniso), 1.0f, instance.MaxSamplerAnisotropy())
            : 1.0f;
