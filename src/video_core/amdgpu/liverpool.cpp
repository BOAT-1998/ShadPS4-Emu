...
static bool EvaluatePredicate(const Liverpool::PredicationState& state) {
    if (!state.enabled || state.address == 0) {
        return true;
    }

    u64 value = 0;
    if (state.use_64bit) {
        std::memcpy(&value, reinterpret_cast<const void*>(state.address), sizeof(u64));
    } else {
        u32 value32 = 0;
        std::memcpy(&value32, reinterpret_cast<const void*>(state.address), sizeof(u32));
        value = value32;
    }

    return state.draw_if_non_zero ? (value != state.reference) : (value == state.reference);
}
    ...
    while (!ccb.empty()) {
        ...
        if (packet_words > ccb.size()) {
            LOG_ERROR(Lib_GnmDriver,
                      ": PM4 CE packet overruns submission. opcode={} words={} remaining={}",
                      static_cast<u32>(header->type3.opcode.Value()), packet_words, ccb.size());
            ccb = {};
            break;
        }
...
Liverpool::Task Liverpool::ProcessGraphics(std::span<const u32> dcb, std::span<const u32> ccb) {
    ...
    while (!dcb.empty()) {
        ...
        case 3: {
            ...
            LOG_ERROR(Lib_GnmDriver,
                      ": PM4 packet overruns submission. opcode={} words={} remaining={}",
                      static_cast<u32>(header->type3.opcode.Value()), packet_words, dcb.size());
            dcb = {};
            break;
        }
            ...
            const PM4ItOpcode opcode = header->type3.opcode;
            const auto* it_body = reinterpret_cast<const u32*>(header) + 1;
            const bool packet_predicated = header->type3.predicate == PM4Predicate::PredEnable;
            if (packet_predicated && !EvaluatePredicate(predication)) {
            ...
            case PM4ItOpcode::ClearState: {
                ...
                predication = {};
                if (rasterizer) {
                    rasterizer->SetPredication(predication);
                }
                break;
            }
            ...
            case PM4ItOpcode::SetPredication: {
                if (count < 3) {
                    LOG_WARNING(Render, "SET_PREDICATION packet too small ({} dwords)", count);
                    predication = {};
                    if (rasterizer) {
                        rasterizer->SetPredication(predication);
                    }
                ...
                Liverpool::PredicationState new_pred{};
                new_pred.raw_control = it_body[0];
                const u64 base_addr =
                    (static_cast<u64>(it_body[2]) << 32) | (it_body[1] & ~0xFULL);
                const u32 addr_offset = it_body[1] & 0xFu;
                new_pred.address = base_addr + addr_offset;
                new_pred.enabled = new_pred.raw_control != 0 && new_pred.address != 0;
                new_pred.draw_if_non_zero = (new_pred.raw_control & 0x1u) == 0;
                new_pred.use_64bit = count >= 5;
                if (count >= 4) {
                    new_pred.reference = it_body[3];
                    if (new_pred.use_64bit && count >= 5) {
                        new_pred.reference |= static_cast<u64>(it_body[4]) << 32;
                    }
                }

                predication = new_pred;
                if (rasterizer) {
                    rasterizer->SetPredication(predication);
                }
                break;
