...
public:
    ...
    explicit CallbackBackend(LogCallback cb) : callback(std::move(cb)) {}
...
std::atomic<std::shared_ptr<CallbackBackend>> pending_callback_backend{nullptr};
std::atomic_bool color_console_enabled_pending{true};
...
public:
    ...
    static void Initialize(std::string_view log_file) {
        ...
        instance->SetCallbackBackend(pending_callback_backend.load());
        instance->SetColorConsoleBackendEnabled(color_console_enabled_pending.load());
    ...
    void SetCallbackBackend(const std::shared_ptr<CallbackBackend>& backend) {
        callback_backend.store(backend);
    }
...
private:
    ...
    void ForEachBackend(auto lambda) {
        ...
        lambda(file_backend);
        if (auto backend = callback_backend.load()) {
            lambda(*backend);
        }
    }
    ...
    std::atomic<std::shared_ptr<CallbackBackend>> callback_backend{nullptr};
...
void SetColorConsoleBackendEnabled(bool enabled) {
    if (Impl::IsActive()) {
        ...
        color_console_enabled_pending.store(enabled);
    }
}
...
void RegisterCallbackBackend(LogCallback callback) {
    auto backend = std::make_shared<CallbackBackend>(std::move(callback));
    pending_callback_backend.store(backend);
    if (Impl::IsActive()) {
        Impl::Instance().SetCallbackBackend(backend);
    }
}
