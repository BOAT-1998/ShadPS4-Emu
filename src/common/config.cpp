...
static ConfigEntry<bool> isShowBootScreen(true);
...
static ConfigEntry<int> resolutionScale(100);
static ConfigEntry<string> graphicsProfile("PS4");
static ConfigEntry<string> textureQuality("High");
static ConfigEntry<int> anisotropicLevel(16);
static ConfigEntry<bool> enableFxaa(false);
static ConfigEntry<bool> enableSharpen(true);
static ConfigEntry<bool> shaderPreloadCache(true);
static ConfigEntry<int> frameLimit(0);
static ConfigEntry<bool> asyncShaderCompile(true);
static ConfigEntry<bool> enableMtRendering(true);
...
static bool enable_ui_sfx = true;
static bool enable_launcher_bgm = false;
...
bool getUiSoundEffectsEnabled() {
    return enable_ui_sfx;
}

void setUiSoundEffectsEnabled(bool enable) {
    enable_ui_sfx = enable;
}

bool getLauncherBgmEnabled() {
    return enable_launcher_bgm;
}

void setLauncherBgmEnabled(bool enable) {
    enable_launcher_bgm = enable;
}
...
void setShowBootScreen(bool enable, bool is_game_specific) {
    isShowBootScreen.set(enable, is_game_specific);
}
...
void setResolutionScale(int scale, bool is_game_specific) {
    const int clamped = std::clamp(scale, 50, 400);
    resolutionScale.set(clamped, is_game_specific);
}

string getGraphicsProfile() {
    return graphicsProfile.get();
}

void setGraphicsProfile(const string& profile, bool is_game_specific) {
    graphicsProfile.set(profile, is_game_specific);
}

string getTextureQuality() {
    return textureQuality.get();
}

void setTextureQuality(const string& quality, bool is_game_specific) {
    textureQuality.set(quality, is_game_specific);
}

int getAnisotropicLevel() {
    return anisotropicLevel.get();
}

void setAnisotropicLevel(int level, bool is_game_specific) {
    constexpr std::array<int, 5> allowed = {1, 2, 4, 8, 16};
    const bool valid = std::ranges::find(allowed, level) != allowed.end();
    anisotropicLevel.set(valid ? level : 16, is_game_specific);
}

bool getEnableFxaa() {
    return enableFxaa.get();
}

void setEnableFxaa(bool enable, bool is_game_specific) {
    enableFxaa.set(enable, is_game_specific);
}

bool getEnableSharpen() {
    return enableSharpen.get();
}

void setEnableSharpen(bool enable, bool is_game_specific) {
    enableSharpen.set(enable, is_game_specific);
}
...
void setFrameLimit(int fps, bool is_game_specific) {
    constexpr std::array<int, 7> allowed = {0, 30, 45, 60, 120, 144, 240};
    const bool valid = std::ranges::find(allowed, fps) != allowed.end();
    frameLimit.set(valid ? fps : 0, is_game_specific);
}
...
bool getEnableMtRendering() {
    return enableMtRendering.get();
}

void setEnableMtRendering(bool enable, bool is_game_specific) {
    enableMtRendering.set(enable, is_game_specific);
}

void AutoTuneFromHardware(bool is_game_specific) {
    // Gather hardware capabilities
    const auto gpus = hwinfo::getAllGPUs();
    const auto cpus = hwinfo::getAllCPUs();

    const auto vram_bytes =
        std::accumulate(gpus.begin(), gpus.end(), int64_t{0},
                        [](int64_t acc, const hwinfo::GPU& gpu) {
                            return std::max(acc, gpu.memory_Bytes());
                        });
    const int vram_gb = static_cast<int>(vram_bytes / (1024 * 1024 * 1024));

    int physical_cores = 0;
    int logical_cores = 0;
    for (const auto& cpu : cpus) {
        physical_cores = std::max(physical_cores, cpu.numPhysicalCores());
        logical_cores = std::max(logical_cores, cpu.numLogicalCores());
    }

    // Derive a performance profile
    int tuned_scale = 100;
    int tuned_aniso = 8;
    std::string tuned_texture = "High";
    std::string tuned_profile = "PS4";
    bool tuned_fsr = true;
    bool tuned_sharpen = true;
    bool tuned_fxaa = false;
    int tuned_frame_limit = 60;
    bool tuned_hdr = false;
    ...
    if (vram_gb < 4) {
        ...
        tuned_fsr = false;
        tuned_sharpen = false;
        tuned_frame_limit = 30;
    } else if (vram_gb < 6) {
        tuned_profile = "PS4";
        tuned_scale = 90;       // lighter for 4 GB GPUs such as RTX 3050
        tuned_aniso = 4;
        tuned_texture = "Medium";
        tuned_frame_limit = 60;
        tuned_fxaa = true;
        tuned_fsr = true;
        tuned_sharpen = false;
    } else if (vram_gb < 8) {
        tuned_profile = "PS4 Pro";
        tuned_scale = 125;
        tuned_aniso = 8;
        tuned_texture = "High";
        tuned_frame_limit = 60;
    ...
    setGraphicsProfile(tuned_profile, is_game_specific);
    setResolutionScale(tuned_scale, is_game_specific);
    setTextureQuality(tuned_texture, is_game_specific);
    setAnisotropicLevel(tuned_aniso, is_game_specific);
    setFsrEnabled(tuned_fsr, is_game_specific);
    setEnableSharpen(tuned_sharpen, is_game_specific);
    setEnableFxaa(tuned_fxaa, is_game_specific);
    setFrameLimit(tuned_frame_limit, is_game_specific);
    setAllowHDR(tuned_hdr, is_game_specific);

    // Multi-threaded recording and async shader compile scale with CPU capability.
    const bool has_many_threads = physical_cores >= 4 || logical_cores >= 8;
    setEnableMtRendering(has_many_threads, is_game_specific);
    setAsyncShaderCompile(true, is_game_specific);
}
...
void load(const std::filesystem::path& path, bool is_game_specific) {
    ...
    if (!std::filesystem::exists(path, error)) {
        if (!is_game_specific) {
            save(path);
            AutoTuneFromHardware(is_game_specific);
            save(path);
        }
        return;
    }
    ...
    if (data.contains("General")) {
        ...
        isShowBootScreen.setFromToml(general, "showBootScreen", is_game_specific);
    ...
    if (data.contains("GPU")) {
        ...
        resolutionScale.setFromToml(gpu, "resolutionScale", is_game_specific);
    }

    if (data.contains("Graphics")) {
        const toml::value& graphics = data.at("Graphics");

        resolutionScale.setFromToml(graphics, "resolutionScale", is_game_specific);
        graphicsProfile.setFromToml(graphics, "profile", is_game_specific);
        textureQuality.setFromToml(graphics, "textureQuality", is_game_specific);
        anisotropicLevel.setFromToml(graphics, "anisotropicLevel", is_game_specific);
        enableFxaa.setFromToml(graphics, "enableFxaa", is_game_specific);
        enableSharpen.setFromToml(graphics, "enableSharpen", is_game_specific);
        shaderPreloadCache.setFromToml(graphics, "shaderPreloadCache", is_game_specific);
        frameLimit.setFromToml(graphics, "frameLimit", is_game_specific);
        asyncShaderCompile.setFromToml(graphics, "asyncShaderCompile", is_game_specific);
        enableMtRendering.setFromToml(graphics, "enableMtRendering", is_game_specific);
    }
    ...
    if (data.contains("GUI")) {
        ...
        enable_ui_sfx = toml::find_or<bool>(gui, "enable_ui_sfx", enable_ui_sfx);
        enable_launcher_bgm =
            toml::find_or<bool>(gui, "enable_launcher_bgm", enable_launcher_bgm);
...
void sortTomlSections(toml::ordered_value& data) {
    ...
    std::vector<string> section_order = {"General", "Input", "Audio", "GPU",     "Graphics",
...
void save(const std::filesystem::path& path, bool is_game_specific) {
    ...
    isShowBootScreen.setTomlValue(data, "General", "showBootScreen", is_game_specific);
    ...
    resolutionScale.setTomlValue(data, "GPU", "resolutionScale", is_game_specific);

    // Graphics quality
    resolutionScale.setTomlValue(data, "Graphics", "resolutionScale", is_game_specific);
    graphicsProfile.setTomlValue(data, "Graphics", "profile", is_game_specific);
    textureQuality.setTomlValue(data, "Graphics", "textureQuality", is_game_specific);
    anisotropicLevel.setTomlValue(data, "Graphics", "anisotropicLevel", is_game_specific);
    enableFxaa.setTomlValue(data, "Graphics", "enableFxaa", is_game_specific);
    enableSharpen.setTomlValue(data, "Graphics", "enableSharpen", is_game_specific);
    shaderPreloadCache.setTomlValue(data, "Graphics", "shaderPreloadCache", is_game_specific);
    frameLimit.setTomlValue(data, "Graphics", "frameLimit", is_game_specific);
    asyncShaderCompile.setTomlValue(data, "Graphics", "asyncShaderCompile", is_game_specific);
    enableMtRendering.setTomlValue(data, "Graphics", "enableMtRendering", is_game_specific);
    ...
    if (!is_game_specific) {
        ...
        data["GUI"]["enable_ui_sfx"] = enable_ui_sfx;
        data["GUI"]["enable_launcher_bgm"] = enable_launcher_bgm;
...
void setDefaultValues(bool is_game_specific) {
    ...
    isShowBootScreen.set(true, is_game_specific);
    ...
    shaderPreloadCache.set(true, is_game_specific);
    resolutionScale.set(100, is_game_specific);
    graphicsProfile.set("PS4", is_game_specific);
    textureQuality.set("High", is_game_specific);
    anisotropicLevel.set(16, is_game_specific);
    enableFxaa.set(false, is_game_specific);
    enableSharpen.set(true, is_game_specific);
    frameLimit.set(0, is_game_specific);
    asyncShaderCompile.set(true, is_game_specific);
    enableMtRendering.set(true, is_game_specific);
