...
// fmt library formatter specialization for BitField
#include <fmt/format.h>

template <std::size_t Position, std::size_t Bits, typename T>
struct fmt::formatter<BitField<Position, Bits, T>>
    : formatter<typename BitField<Position, Bits, T>::UnderlyingType> {
    template <typename FormatContext>
    auto format(const BitField<Position, Bits, T>& bf, FormatContext& ctx) const {
        return formatter<typename BitField<Position, Bits, T>::UnderlyingType>::format(bf.Value(),
                                                                                       ctx);
    }
};
